(() => {
  ...
  function showTheftAlert(){
    ...
    requestAnimationFrame(()=>ui.theftAlert?.classList.add("show"));
  ...
  function updateHUD(force=false){
    ...
    if(bossVisible){const display=boss.name==="karel"?"KRYSTALOVÝ KAREL":"FETÁK FRANTA";if(ui.bossName)ui.bossName.textContent=display;if(ui.bossFill)ui.bossFill.style.width=`${clamp((boss.maxHits-boss.hits)/boss.maxHits*100,0,100)}%`;if(ui.bossPhase)ui.bossPhase.textContent=boss.stunTimer>0?"ZRANITELNÝ · CHYŤ HO":boss.dashTime>0?"SPRINTUJE":boss.phase>=3?"ZUŘIVÁ FÁZE":boss.phase===2?"ZRYCHLUJE":"VYČKEJ NA PAUZU";}
  ...
  function finishDig(){
    ...
    if(h.special==="hedgehog"){
      ...
      setTimeout(()=>{startRival("karel",h.x+150,h.y-90);toast("Dožeň zloděje ve chvíli, kdy se zastaví!","bad",2200);},520);
      audio.sfx("rare");
    }else{
  ...
  function startRival(name,x,y){
    ...
    world.rival={name,displayName:name==="karel"?"KRYSTALOVÝ KAREL":"FETÁK FRANTA",x,y,r:30,hits:0,maxHits:name==="karel"?3:2,speed:name==="karel"?150:166,baseSpeed:name==="karel"?150:166,angle:0,target:{x:rand(250,1550),y:rand(220,950)},throwTimer:1.15,active:true,flashlight:name==="karel",vision:name==="karel"?245:0,baseVision:name==="karel"?245:0,halfAngle:name==="karel"?.5:0,seesPlayer:false,phase:1,hitFlash:0,dashTimer:1.8,dashTime:0,stunTimer:0,trail:[]};
    bossIntroTimer=2.35;
  ...
  function update(dt){
    ...
    theftAlertTimer=Math.max(0,theftAlertTimer-dt);if(theftAlertTimer<=0){ui.theftAlert?.classList.remove("show");setTimeout(()=>ui.theftAlert?.classList.add("hidden"),180);}dangerBeatTimer=Math.max(0,dangerBeatTimer-dt);state.comboTimer=Math.max(0,state.comboTimer-dt);if(state.comboTimer<=0&&state.combo>1){state.combo--;state.comboTimer=5;}
  ...
  function updateRival(dt){
    ...
    if(r&&r.active){
      ...
      r.dashTimer-=dt;
      if(r.trail){r.trail.unshift({x:r.x,y:r.y,life:.34});for(const t of r.trail)t.life-=dt;r.trail=r.trail.filter(t=>t.life>0).slice(0,8);}
      if(r.dashTime>0){
        r.dashTime-=dt;
        ...
        if(r.name==="karel"&&r.dashTimer<=0){r.dashTimer=Math.max(1.25,2.5-r.phase*.35)+Math.random()*.55;r.dashTime=.42+.06*r.phase;r.angle=Math.atan2(player.y-r.y,player.x-r.x)+pick([-.72,.72]);audio.sfx("alert");}
      }
      r.x=clamp(r.x,80,1720);r.y=clamp(r.y,100,1120);
      r.seesPlayer=false;
      if(r.flashlight&&r.stunTimer<=0&&insideVisionCone(r,r.vision,r.halfAngle)){
        r.seesPlayer=true;markDanger("Karlova svítilna",r.phase>=3?72:r.phase===2?64:58,r.phase>=3?1.15:r.phase===2?1.35:1.55);
      }
      r.throwTimer-=dt;
      if(r.throwTimer<=0&&r.stunTimer<=0&&r.dashTime<=0){r.throwTimer=Math.max(.48,1.15-r.phase*.18)+Math.random()*.42;const aim=Math.atan2(player.y-r.y,player.x-r.x);world.hazards.push({type:"clod",x:r.x,y:r.y,vx:Math.cos(aim)*150*(1+r.phase*.08),vy:Math.sin(aim)*150*(1+r.phase*.08),life:2.2,r:10+r.phase});}
    }
